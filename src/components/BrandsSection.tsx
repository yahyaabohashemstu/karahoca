...
/**
 * Master switch for the private-label / custom-manufacturing promo — the
 * "From Your Idea to a Market-Ready Product" card together with the partner
 * logo strip inside it (`.brands-partner-row`).
 *
 * Currently off, in every language. Nothing was deleted: the JSX below, the
 * `customOrder.*` keys in all four locale files, the `.brands-partner-*` and
 * `.brands-logo-*` rules in styles/main.css, the PARTNERS array above and
 * `whatsAppCustomOrderUrl` are all deliberately retained. Flipping this to
 * `true` restores the block everywhere — no other edit required.
 *
 * Two things to preserve if you touch this:
 *   - Guard the inner `.brands-partner-row` div, NOT the `<section>` — the
 *     `#brands` id lives on the section and the header, hero CTA and footer
 *     all scroll to it.
 *   - Keep the JSX behind `&&` rather than commenting it out. It still
 *     type-checks that way, which is what keeps PARTNERS, `i18n` and
 *     `whatsAppCustomOrderUrl` counting as used under `noUnusedLocals`.
 */
const SHOW_CUSTOM_ORDER_SECTION = false;
