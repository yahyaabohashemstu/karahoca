...
export const BLOG_BATCH_2 = [
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    excerpt: {
      ar: '٤ مشاكل تواجه كلّ حمّام تقريباً — ترسّبات الكلس، العفن الأسود بين البلاط، الروائح من البالوعة، وبقع الصدأ — وكيف نحلّها كيميائياً وليس فقط بفركٍ يدوي.',
...
## ١. ترسّبات الكلس (Limescale) — الطبقة البيضاء/الصفراء
...
## ٢. العفن الأسود بين البلاط (Grout Mould)
...
**الحلّ:** **محلول مبيّض كلوري بتركيز ٥-٧٪** يقتل العفن من جذوره:
...
- اتركه **٢٠ دقيقة كاملة** — هذا الوقت ضروري لقتل الأبواغ
...
**هل تدير فندقاً أو منشأة كبيرة؟** نوفّر **AYLUX منظف حمّام** بعبوات ٥ لتر للجملة. [راسلنا للحصول على سعر العقد الشهري](mailto:info@karahoca.com).`,
...
**Управляете отелем или большим объектом?** Мы поставляем **AYLUX средство для ванной** в канистрах 5 л оптом. [Свяжитесь с нами для ценового предложения](mailto:info@karahoca.com).`,
  ...
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    title: {
      ar: '٧ أخطاء شائعة في الغسيل تختصر عمر ملابسك للنصف',
    ...
    excerpt: {
      ar: 'دراسات الصناعة تقول إنّ ٧٠٪ من تلف الأقمشة سببه طريقة الغسيل، وليس عمر الثوب. هذه ٧ أخطاء يفعلها الجميع تقريباً، وكيف نتجنّبها.',
    ...
    body: {
      ar: `كم مرّة قلت "هذا القميص لم يدم طويلاً"؟ في الواقع، أكثر تلف الأقمشة لا يحصل من اللبس — يحصل من الغسّالة. هذه ٧ أخطاء يفعلها معظم الناس.

## ❌ الخطأ ١: استخدام كمّية مسحوق مضاعفة
...
- غسلة عادية: ٧٠ غرام
- غسلة قذرة جداً: ١٠٠ غرام (الحدّ الأقصى)

## ❌ الخطأ ٢: درجة حرارة خاطئة

الكثير يغسل كلّ شيء على ٦٠ درجة "للتعقيم". المشكلة: درجات الحرارة العالية:
...
|-----------|-----------------|
| دم، عرق، بيض | **ماء بارد** (٢٠-٣٠°) |
| طعام، شحوم | ٤٠° |
| ملاءات وفوط بيضاء | ٦٠° |
| ملابس داخلية (تعقيم) | ٩٠° |

## ❌ الخطأ ٣: تجميع الغسيل لأسبوع كامل

البقع الطازجة (أقلّ من ٢٤ ساعة) تخرج بسهولة. بعد أسبوع، البقعة تتأكسد وتلتصق كيميائياً بألياف القماش.

**الصحيح:** اغسل البقع الجديدة في يومها. لو لا تستطيع، رشّ المنطقة بـ **DIOX مزيل بقع** فوراً ودعها مبتلّة حتّى يوم الغسيل.

## ❌ الخطأ ٤: تعبئة الغسّالة لآخرها
...
**الصحيح:** املأ الحلّة بـ **٧٠٪** من سعتها. عند إغلاق الباب، يجب أن تستطيع وضع كفّ يدك أفقياً فوق الملابس.

## ❌ الخطأ ٥: عدم فصل الألوان
...
## ❌ الخطأ ٧: تجاهل **مطري الأقمشة**

الكثيرون يعتبرون مطري الأقمشة "كمالي". في الحقيقة، له ٣ فوائد عملية:
1. **يقلّل التجاعيد** = أقلّ كيّ
2. **يمنع الكهرباء الساكنة** في الشتاء
3. **يحمي الألياف** من الاحتكاك في الغسلة

**منتجنا الموصى به:** **DIOX مطري الغسيل** — يأتي بـ ٦ نكهات معطّرة + يحوي عناصر تطيل عمر القماش.

## 💡 ملخّص سريع للذاكرة

> **القاعدة الذهبية:** ماء بارد + كمّية موصى بها من المسحوق + ٧٠٪ ملء + فصل الألوان + نشر سريع.

اتباع هذا الـ workflow يمدّ عمر ملابسك **٢-٣ سنوات إضافية**.
...
**Управляете прачечной или отелем?** Предлагаем промышленные формулы **DIOX** с повышенной концентрацией. [Закажите бесплатный образец](mailto:info@karahoca.com).`,
  ...
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    excerpt: {
      ar: 'الأطفال يلعقون الأرض، والقطط تلمس بأقدامها كلّ سطح ثمّ تلعقها. هذا الدليل يحدّد ٧ مكوّنات خطرة ينبغي تجنّبها، وبدائل آمنة معتمدة من EU وFDA.',
...
## ⚠️ ٧ مكوّنات يجب تجنّبها

### ١. **Sodium Hypochlorite** (الكلور)
...
\`\`\`
هل في المنزل أطفال < ٣ سنوات أو حيوانات؟
├── نعم
...
2. ضع نقطتين من سائل غسيل الصحون + ضع الملابس بماء بارد ٣٠ دقيقة
...
### الشحوم والزيوت
1. **رشّ بودرة بيكنغ صودا** على البقعة لـ١٥ دقيقة (تمتصّ الزيت)
2. أزل البودرة بقطعة قماش جافّة
3. ضع نقطة من سائل غسيل الصحون مباشرة + افرك
4. اغسل بـ **AYLUX سائل غسيل** على ٤٠°

### الكاتشب والصلصة
1. اكشط الزائد بسكين بلاستيكي (لا تستخدم قماش)
2. اشطف بماء بارد من الخلف
3. خفّف خلّاً أبيض + ماء بنسبة 1:1 وضع على البقعة لـ ١٠ دقائق
4. اغسل عادياً
...
2. مزيج: ٣ ملاعق Hydrogen Peroxide ٣٪ + ١ ملعقة سائل غسيل الصحون
3. ضع المزيج على البقعة لـ ٥ دقائق
4. اغسل بماء بارد + **DIOX سائل غسيل**

**بقع دم قديمة (أكثر من ٤٨ ساعة):**
...
3. اتركه ٣٠ دقيقة
4. اغسل بـ **DIOX مسحوق أوتوماتيك** على ٦٠° (للقطن فقط)
...
2. ضع بيكنغ صودا + خلّ على المنطقة ٢٠ دقيقة (الإنزيمات تحلّل البروتين)
3. اغسل بـ **DIOX سائل غسيل** على ٣٠°
...
**حلّ لجميع البقع في زجاجة واحدة:** [DIOX مزيل بقع](/ar/diox) — تركيبة إنزيمية تفكّك ٢٠+ نوع بقعة بدون فرك مكثّف.`,
